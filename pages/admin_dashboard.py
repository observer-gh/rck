...
# --- Utility Functions ---
# These should eventually be moved to a shared utility module.
...
def render_analytics_tab():
    ...
    if club_points:
        ...
        top_clubs = sorted(club_points.items(), key=lambda item: item[1], reverse=True)[:5]

        leader_names = []
        points = []
        for cid, pts in top_clubs:
            club = clubs_map.get(cid)
            leader_name = _user_name(club['leader_id'], user_map) if club else '?'
...
def render_matching_tab():
    ...
    if st.button("매칭 실행 / 새 버전 생성"):
        if len(users_raw) < target_size:
            ...
            run_meta = MatchRun(id=run_id, created_at=utc_now_iso(), target_size=target_size, user_count=len(users_raw), club_count=len(clubs_dicts))
            runs.append(asdict(run_meta))
            persistence.replace_all('match_runs', runs)

            st.success(f"새 매칭 실행 완료. Run ID: {run_id}, 생성된 클럽 수: {len(clubs_dicts)}")
            st.balloons()
...
def render_clubs_tab():
    ...
    if run_ids:
        ...
        clubs_to_display = [c for c in clubs_all if c.get('match_run_id') == selected_run_id]
    else:
    ...
    for idx, c in enumerate(clubs_to_display, start=1):
        ...
        with st.expander(club_title):
            ...
            member_names = [_user_name(mid, user_map) for mid in c['member_ids']]
...
def render_data_tab():
    ...
    with st.container(border=True):
        ...
        c1, c2, c3, c4 = st.columns(4)
        if c1.button("사용자"):
            st.download_button("다운로드: users.csv", _to_csv(persistence.load_list('users')), "users.csv", "text/csv")
        if c2.button("클럽"):
            st.download_button("다운로드: clubs.csv", _to_csv(persistence.load_list('clubs')), "clubs.csv", "text/csv")
        if c3.button("보고서"):
            st.download_button("다운로드: reports.csv", _to_csv(persistence.load_list('activity_reports')), "reports.csv", "text/csv")
        if c4.button("매칭기록"):
            st.download_button("다운로드: runs.csv", _to_csv(persistence.load_list('match_runs')), "runs.csv", "text/csv")
