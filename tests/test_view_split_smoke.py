...
def test_admin_pages_are_correctly_flagged():
    ...
    """
    admin_pages = [key for key, value in PAGE_REGISTRY.items() if value["admin"]]
...
def test_user_pages_are_correctly_flagged():
    ...
    """
    user_pages = [key for key, value in PAGE_REGISTRY.items() if not value["admin"]]
    expected_user_pages = ["user_signup", "my_club", "activity_report", "demo_script"]
...
def test_all_render_functions_are_callable():
    ...
    for page_key, page_config in PAGE_REGISTRY.items():
        assert callable(page_config['render_func']), f"Render function for '{page_key}' is not callable."